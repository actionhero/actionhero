describe('Core: Cache', function(){
  ...
  it('cache.save', function(done){
  	apiObj.cache.save(apiObj,"testKey","abc123",null,function(resp){
  ...
  it('cache.load', function(done){
  	apiObj.cache.load(apiObj,"testKey",function(resp){
  ...
  it('cache.load failures', function(done){
  	apiObj.cache.load(apiObj,"something else",function(resp){
  ...
  it('cache.destroy', function(done){
  	apiObj.cache.destroy(apiObj,"testKey",function(resp){
  		resp.should.equal(true);
  		done();
  	});
  });
  ...
  it('cache.save with expire time', function(done){
  	apiObj.cache.save(apiObj,"testKey","abc123",10,function(resp){
  		resp.should.equal(true);
  		done();
  	});
  });
  ...
  it('cache.load with expired items should not return them', function(done){
  	apiObj.cache.save(apiObj,"testKey_slow","abc123",10,function(save_resp){
  		save_resp.should.equal(true);
  		setTimeout(function(){
	  		apiObj.cache.load(apiObj,"testKey_slow",function(load_resp){
  ...
  it('cache.load with negative expire times will never load', function(done){
  	apiObj.cache.save(apiObj,"testKeyInThePast","abc123",-1,function(save_resp){
  		save_resp.should.equal(true);
  		apiObj.cache.load(apiObj,"testKeyInThePast",function(load_resp){
	  		should.equal(null, load_resp);
	  		done();
	  	});
  	});
  ...
  it('cache.save does not need to pass expireTime', function(done){
  	apiObj.cache.save(apiObj,"testKeyForNullExpireTime","abc123",function(save_resp){
  		save_resp.should.equal(true);
  		apiObj.cache.load(apiObj,"testKeyForNullExpireTime",function(load_resp){
  ...
  it('cache.save works with arrays', function(done){
  	apiObj.cache.save(apiObj,"array_key",[1,2,3],function(save_resp){
  		save_resp.should.equal(true);
  		apiObj.cache.load(apiObj,"array_key",function(load_resp){
  ...
  it('cache.save works with objects', function(done){
	...
	data.otherThing = [1,2,3];
  	apiObj.cache.save(apiObj,"obj_key",data,function(save_resp){
  		save_resp.should.equal(true);
  		apiObj.cache.load(apiObj,"obj_key",function(load_resp){
