...
cache.buildSQLExpireString = function(api, secondsFromNow){
	var now = new Date();
...
cache.save = function(api,key,value,expireTimeSeconds,next){
	if(expireTimeSeconds < 0 || expireTimeSeconds == null){ expireTimeSeconds = api.configData.cache.defaultExpireTimeSeconds; }
	var expireTimeString = cache.buildSQLExpireString(api, expireTimeSeconds);
