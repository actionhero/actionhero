 ...
 */
export class Config extends Initializer {
  constructor() {
    super();
    this.name = "config";
    this.loadPriority = 1;
  ...
  async initialize() {
    ...
    if (configPaths.length < 1) {
      throw new Error(
        configPaths +
          "No config directory found in this project, specified with --config, or found in process.env.ACTIONHERO_CONFIG"
      );
    }
    ...
    api.loadConfigDirectory = (configPath, watch) => {
      ...
      if (api.config.routes) {
        Object.keys(api.config.routes).forEach(v => {
          api.config.routes[v] = api.config.routes[v].filter(
            (route, index, self) =>
              index ===
              self.findIndex(
                r =>
    ...
    if (api._startingParams && api._startingParams.configChanges) {
      api.config = api.utils.hashMerge(
        api.config,
        api._startingParams.configChanges
      );
    }
  }
