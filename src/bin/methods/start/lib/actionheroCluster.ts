...
export class ActionHeroCluster {
  ...
  clearPidFile() {
    ...
    if (fs.existsSync(file)) {
      const filePid = parseInt(fs.readFileSync(file).toString());
  ...
  async stop() {
    ...
    if (this.workers.length > 0) {
      this.log(
        this.workers.length + " workers running, waiting on stop",
        "info"
      );
  ...
  work() {
    ...
    if (
      ...
      if (stateCounts.started === this.workers.length) {
        this.log(
          "cluster equilibrium state reached with " +
            this.workers.length +
            " workers",
          "notice"
        );
